...
class ApplicantCompanyLabelsQuery
{
    ...
    public function enabled($_, array $args)
    {
        $args = DB::table('applicant_company_labels as ail')
            ->select('*',DB::raw('CASE
                WHEN ailr.applicant_company_id is not null and ailr.applicant_company_id = '.$args['applicant_id'].' THEN true
                WHEN ailr.applicant_company_id = '.$args['applicant_id'].'  THEN true
                ELSE false
                END AS is_active'))
