...
class Account extends BaseModel implements BaseModelInterface
{
    ...
    public static function getAccountRequisitesFilter($query, $filter)
    {
        $sql = self::join('companies', 'accounts.company_id', '=', 'companies.id')
            ->join('group_role', 'accounts.group_role_id', '=', 'group_role.id')
            ->join('applicant_individual', 'accounts.owner_id', '=', 'applicant_individual.id')
            ->join('payment_provider', 'accounts.payment_provider_id', '=', 'payment_provider.id')
            ->join('payment_system', 'accounts.payment_system_id', '=', 'payment_system.id')
            ->join('payment_banks', 'accounts.payment_bank_id', '=', 'payment_banks.id')
            ->select('accounts.*')
            ->where(function ($q) use ($query) {
                $q->orWhere('accounts.id', 'like', $query['account_name'] ?? '%')
                    ->orWhere('accounts.account_name', 'like', $query['account_name'] ?? '%');
            ...
            ->where(function ($q) use ($filter) {
                $q->orWhere('payment_system.id', 'like', $filter['payment_system'] ?? '%')
                    ->orWhere('payment_system.name', 'like', $filter['payment_system'] ?? '%');
            })
            ->where(function ($q) use ($filter) {
                $q->orWhere('payment_banks.id', 'like', $filter['payment_bank'] ?? '%')
                    ->orWhere('payment_banks.name', 'like', $filter['payment_bank'] ?? '%');
            })
            ->where(function ($q) use ($filter) {
        ...
        if (isset($query['id'])) {
            $sql = $sql->where('accounts.id', '=', $query['id']);
        }
