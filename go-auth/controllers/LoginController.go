...
func Login(context *gin.Context) {
	...
	oauthRepository.InsertAuthLog(clientType, user.GetEmail(), user.GetCompany().Name, constants.StatusLogin, expirationJWTTime, deviceInfo)
	oauthRepository.InsertActiveSessionLog(clientType, user.GetEmail(), true, true, expirationJWTTime, deviceInfo)
...
func checkAndUpdateSession(provider string, email string, fullName string, companyId uint64, deviceInfo *dto.DeviceDetectorInfo, c *gin.Context) (bool, int, gin.H) {
	activeSessionCreated := oauthRepository.InsertActiveSessionLog(provider, email, true, false, time.Now().UTC(), deviceInfo)
